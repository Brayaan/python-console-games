...
# Activar soporte de códigos ANSI en Windows (necesario para los colores en cmd/PowerShell)
if os.name == "nt":
    import ctypes
    try:
        ...
        pass  # Si falla (Windows muy antiguo), los colores simplemente no se muestran

# Cambia a True para ver mensajes de depuración en consola
DEBUG = False

# Puerto UDP del beacon del servidor (debe coincidir con PUERTO_BEACON en servidor.py)
PUERTO_BEACON = 8889
...
class ClienteJuegos:
    ...
    Mantiene la conexión TCP, lanza un hilo que escucha mensajes
    ...
    def __init__(self, host=None, puerto=8888):
        self.host = host             # Se asigna en menu_conexion(); None hasta entonces
        self.puerto = puerto
    ...
    @staticmethod
    def descubrir_servidores(timeout=2.0):
        """Busca servidores activos en la red local mediante UDP broadcast.

        Envía el paquete 'DISCOVER_SERVER' a la dirección de broadcast de la LAN
        y recoge las respuestas 'SERVER_RESPONSE:<ip>:<puerto>' durante `timeout`
        segundos. Las respuestas duplicadas se descartan.

        Args:
            timeout: Tiempo máximo en segundos para esperar respuestas (por defecto 2).

        Returns:
            Lista de tuplas (ip: str, puerto: int) con los servidores encontrados.
            Lista vacía si no se encuentra ninguno o si el broadcast falla.
        """
        servidores = []
        vistos = set()  # Para evitar duplicados si el servidor responde varias veces
        try:
            sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
            sock.setsockopt(socket.SOL_SOCKET, socket.SO_BROADCAST, 1)
            sock.settimeout(timeout)
            sock.sendto(b"DISCOVER_SERVER", ("255.255.255.255", PUERTO_BEACON))

            fin = time.time() + timeout
            while time.time() < fin:
                try:
                    datos, _ = sock.recvfrom(1024)
                    msg = datos.decode("utf-8", errors="ignore").strip()
                    if msg.startswith("SERVER_RESPONSE:"):
                        partes = msg.split(":")         # ['SERVER_RESPONSE', 'ip', 'puerto']
                        if len(partes) == 3:
                            ip  = partes[1]
                            prt = int(partes[2])
                            clave = (ip, prt)
                            if clave not in vistos:
                                vistos.add(clave)
                                servidores.append(clave)
                except socket.timeout:
                    break
                except Exception:
                    pass
        except Exception:
            pass  # Si el broadcast falla (red bloqueada) retornar lista vacía
        finally:
            try:
                sock.close()
            except Exception:
    ...
    def menu_conexion(self):
        """Muestra el menú interactivo de selección de servidor antes de conectar.

        Realiza el descubrimiento UDP automático de servidores en la red local.
        Muestra claramente cada servidor encontrado con su IP y estado EN LÍNEA.
        Siempre ofrece la opción de ingresar la IP manualmente como fallback.

        Returns:
            True si el usuario seleccionó un servidor y la conexión fue exitosa.
            False si el usuario eligio salir.
        """
        # Pantalla de búsqueda mientras se escanea la red
        ...
        print(f"{Colores.AMARILLO}  Buscando servidores en la red...{Colores.RESET}\n")

        servidores = self.descubrir_servidores(timeout=2.0)
        ...
        while True:
            ...
            opciones_ip = []  # Lista paralela de (ip, puerto) para cada opción numerada

            if servidores:
                # Mostrar cada servidor con bloque visual claro e IP destacada
                for ip, prt in servidores:
                    print(f"  {Colores.VERDE}┌─ Servidor disponible ──────────────┐{Colores.RESET}")
                    print(f"  {Colores.VERDE}│{Colores.RESET}  IP:    {Colores.BOLD}{Colores.VERDE}{ip}{Colores.RESET}")
                    print(f"  {Colores.VERDE}│{Colores.RESET}  Puerto: {Colores.BOLD}{prt}{Colores.RESET}")
                    print(f"  {Colores.VERDE}│{Colores.RESET}  Estado: {Colores.VERDE}{Colores.BOLD}EN LÍNEA  ✓{Colores.RESET}")
                    print(f"  {Colores.VERDE}└────────────────────────────────────┘{Colores.RESET}")
                    print()
                    opciones_ip.append((ip, prt))
            else:
                print(f"  {Colores.AMARILLO}No se encontraron servidores en la red.{Colores.RESET}")
                print()

            # Opciones numeradas: primero cada servidor, luego manual y buscar
            for i, (ip, _) in enumerate(opciones_ip, start=1):
                print(f"  {Colores.VERDE}{i}.{Colores.RESET} Conectar a {Colores.BOLD}{ip}{Colores.RESET}")
            n_manual = len(opciones_ip) + 1
            n_buscar = len(opciones_ip) + 2
            print(f"  {Colores.AZUL}{n_manual}.{Colores.RESET} Ingresar IP manualmente")
            print(f"  {Colores.AZUL}{n_buscar}.{Colores.RESET} Buscar de nuevo")
            print(f"  {Colores.ROJO}0.{Colores.RESET}  Salir")
            print()

            try:
                opcion = input(f"{Colores.VERDE}►{Colores.RESET} ").strip()
            ...
            # Conectar a servidor seleccionado de la lista
            if 1 <= idx <= len(opciones_ip):
                ip_sel, prt_sel = opciones_ip[idx - 1]
                self.host   = ip_sel
                self.puerto = prt_sel
                print(f"\n{Colores.AMARILLO}  Conectando a {ip_sel}:{prt_sel}...{Colores.RESET}")
                if self.conectar():
                    return True
                print(f"{Colores.ROJO}[!] No se pudo conectar. Intenta de nuevo.{Colores.RESET}")
                time.sleep(1.5)
                continue
            ...
            if idx == n_manual:
                try:
                    ip_manual = input(
                        f"{Colores.VERDE}  IP del servidor (Enter para cancelar):{Colores.RESET} "
                    ).strip()
                except (EOFError, KeyboardInterrupt):
                    continue
                if not ip_manual:
                ...
                try:
                    puerto_str = input(
                        f"{Colores.VERDE}  Puerto [{self.puerto}] (Enter = {self.puerto}):{Colores.RESET} "
                    ).strip()
                except (EOFError, KeyboardInterrupt):
                    continue
                ...
                self.host = ip_manual
                print(f"\n{Colores.AMARILLO}  Conectando a {self.host}:{self.puerto}...{Colores.RESET}")
                if self.conectar():
                    return True
                print(f"{Colores.ROJO}[!] No se pudo conectar. Verifica la IP e intenta de nuevo.{Colores.RESET}")
                time.sleep(1.5)
                continue
    ...
    def procesar_mensaje(self, mensaje):
        ...
        if tipo == "LISTA_JUEGOS":
            ...
            self.en_partida = False
            # Desbloquear cualquier wait activo (evita colgarse en POSIX/Android)
            self.esperando_actualizacion.set()
            print("")
    ...
    def jugar_turno(self):
        ...
        if mi_turno:
            ...
            try:
                ...
                return
            # Verificar que la partida no terminó mientras escribiamos
    ...
    def _esperar_turno_rival(self):
        ...
        En POSIX (Linux/Mac/Android) usa select() para el mismo efecto sin bloquear.
        ...
        if os.name == "nt":
            ...
            # POSIX (Linux / Mac / Android-Termux): usar select() para polling
            # no bloqueante. Permite detectar en_partida=False aunque el usuario
            # no haya presionado Enter, resolviendo el bug de pantalla congelada.
            import select
            prompt = f"\n{Colores.AMARILLO}Esperando al rival... ('C' chatear, 'rendirse' abandonar): {Colores.RESET}"
            print(prompt, end="", flush=True)
            while self.en_partida and self.conectado:
                # Si ya es nuestro turno (el rival movio), salir
                if self.juego_actual and self.juego_actual.get("turno") == self.jugador_id:
                    print()
                    return ""
                # Esperar hasta 0.1s si hay datos en stdin; si no, continuar el bucle
                # y volver a evaluar en_partida — esto es lo que faltaba antes
    ...
    def iniciar(self):
        ...
        Muestra el menú de descubrimiento/conexión, registra el nombre del
        jugador y arranca el bucle del menú principal. Si el usuario elige salir
        o la conexión falla, termina sin lanzar excepción.
        """
        if not self.menu_conexion():
            return  # El usuario eligió salir o no se pudo conectar
...
if __name__ == "__main__":
    # Si se pasan argumentos de línea de comandos (ip [puerto]), se saltan el
    # menú de descubrimiento y se conecta directamente (modo avanzado/depuración).
    if len(sys.argv) > 1:
        host   = sys.argv[1]
        ...
        # Modo normal: menú de descubrimiento automático
