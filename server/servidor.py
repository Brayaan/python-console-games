    ...
    def procesar_movimiento(self, jugador_id, mensaje):
        ...
        if valido:
            if resultado.get("terminado"):
                ...
                # Limpiar sala DESPUÉS del broadcast (sin lock: _terminar_sala adquiere el suyo)
        ...
        with self.lock:
            ...
            if sala_id and sala_id in self.salas:
                ...
                if oponente and oponente in self.jugadores:
                    ...
                    # Notificar al oponente SIEMPRE (no solo cuando estado==JUGANDO)
                    # Así el cliente Android/POSIX no queda colgado esperando
                    try:
